...
public class Ex3_Runner {
    public static void main(String[] args) throws Exception {
        if (args.length != 5) {
            System.err.println("Usage: Ex3_Runner <movies> <users> <rating_1> <rating_2> <output>");
            System.exit(1);
        }

        Configuration conf = new Configuration();
        Job job = Job.getInstance(conf, "Exercise 3 Average Rating by Gender");
        job.setJarByClass(Ex3_Runner.class);
